...
var Core = function(conf) {
    ...
    this._isWebgl2 = conf.isWebgl2 == undefined ? false : conf.isWebgl2;
    ...
    this._datasets_paths = conf.datasets_paths;
...
Core.prototype.init = function() {
    ...
    if (this._isWebgl2) {
      console.log("WebGL2 mode is active");
    ...
    if(this._mode == "2d") {
      ...
      if (this._isWebgl2) {
        this._materialFirstPass = new THREE.RawShaderMaterial( {
          vertexShader: this._shaders.firstPass2.vertexShader,
          fragmentShader: this._shaders.firstPass2.fragmentShader,
          side: THREE.FrontSide,
          //side: THREE.BackSide,
          transparent: true
        });

        this._materialSecondPass = new THREE.RawShaderMaterial( {
          vertexShader: this._shaders[this._shader_name + "2"].vertexShader,
          fragmentShader: ejs.render( this._shaders[this._shader_name + "2"].fragmentShader, {
            "maxTexturesNumber": me.getMaxTexturesNumber()}),
            uniforms: {
              uRatio : { type: "f", value: this.zFactor},
              uBackCoord: { type: "t",  value: this._rtTexture },
              uSliceMaps: { type: "t", value: this._slicemaps_textures },
...
Core.prototype._setSlicemapsTextures = function(images) {
    ...
    if (this._isWebgl2) {
      var gl2 = this._render.context;
      var texture3D = gl2.createTexture();

      var xhr = new XMLHttpRequest();
      xhr.open('GET', this._datasets_paths[0], true);
      ...
      xhr.onload = function(e) {
        ...
        if (arrayBuffer) {
          ...
          // var dataRes = Math.round(Math.pow(byteArray.length, 1/3));

          gl2.bindTexture(gl2.TEXTURE_3D, texture3D);
          ...
          gl2.texImage3D(gl2.TEXTURE_3D, 0, gl2.LUMINANCE, 256, 256, 449, 0, gl2.LUMINANCE, gl2.UNSIGNED_BYTE, byteArray);
        }
      }
      xhr.send(null);
      this._slicemaps_textures = texture3D;
...
window.VRC.Core.prototype._shaders.firstPass2 = {
	uniforms: THREE.UniformsUtils.merge([
		{
	...
	vertexShader: [
		...
		'{',
		'    //Set the world space coordinates of the back faces vertices as output.',
		'    worldSpaceCoords = position.xyz + vec3(0.5, 0.5, 0.5); //move it from [-0.5;0.5] to [0,1]',
...
window.VRC.Core.prototype._shaders.secondPassDVR2 = {
	uniforms: THREE.UniformsUtils.merge([
		{
		"projectionMatrix" : { type: "m4", value: new THREE.Matrix4( ) },
		"modelViewMatrix" : { type: "m4", value: new THREE.Matrix4( ) },
		"uBackCoord" : { type: "t", value: null },
		"uSliceMaps" : { type: "t", value: null },
		"uOpacityVal" : { type: "f", value: -1 },
		"darkness" : { type: "f", value: -1 },
		"uMinGrayVal" : { type: "f", value: -1 },
		"uMaxGrayVal" : { type: "f", value: -1 },
	...
	fragmentShader: [
		...
		'          colorValue.x = (darkness * 2.0 - gray_val.x) * l * 0.4;',
		'          colorValue.w = 0.1;',
		'          samp.rgb = (1.0 - accum.a) * colorValue.xxx * samp.a;',
		'          samp.a = colorValue.a * opacityFactor * (1.0 / uStepsF);',
		'          accum += samp;',
		'          if(accum.a>=1.0)',
		'             break;',
		'      }',
...
window.VRC.Core.prototype._shaders.secondPassFusion2 = {
	uniforms: THREE.UniformsUtils.merge([
		{
		"projectionMatrix" : { type: "m4", value: new THREE.Matrix4( ) },
		"modelViewMatrix" : { type: "m4", value: new THREE.Matrix4( ) },
	...
	fragmentShader: [
		...
		'          // colorValue.z = gray_val.z;',
		'          samp.a = colorValue.a * opacityFactor * (1.0 / uStepsF);',
		'          samp.rgb = (1.0 - accum.a) * hsv2rgb(colorValue.rgb) * samp.a;',
		'          accum += samp;',
		'          if(accum.a>=1.0)',
		'             break;',
		'   }',
...
window.VRC.Core.prototype._shaders.secondPassSoebel2 = {
	uniforms: THREE.UniformsUtils.merge([
		{
		"projectionMatrix" : { type: "m4", value: new THREE.Matrix4( ) },
		"modelViewMatrix" : { type: "m4", value: new THREE.Matrix4( ) },
	...
	fragmentShader: [
		...
		'uniform float l;',
		'out vec4 fragColor;',
		'const float xw = 256.0;',
		'const float yw = 256.0;',
		'const float zw = 449.0;',
		'// Compute the Normal around the current voxel',
		'vec3 getNormal(vec3 at) {',
		'  vec3 texpos1;',
		'  float w1 = at.z - floor(at.z);',
		...
		'  lightPos[2] = vec3(1, 1, -1);',
		'  for(int i = 0; i < uStepsI; i++) {',
		'      float gray_val = texture(uSliceMaps, vpos.xyz).x;',
		'      /************************************/',
		'      /*         Mean filtering           */',
		'      /************************************/',
		...
		'          }',
		'        }',
		'        gray_val = sum_gray_val / 27.0; // 27.0 = pow(mask_size, 3)',
		'      } // end of Mean filtering',
		'      if(gray_val > uMinGrayVal && gray_val < uMaxGrayVal) {',
		'          vec3 V = normalize(pos.xyz - vpos.xyz);',
		'          vec3 N = normalize(getNormal(vpos.xyz));',
		...
		'            vec3 L = normalize(lightPos[l_i] - vpos.xyz);',
		'            // if (true) { // Blinn-Phong shading mode',
		'                vec3 Iamb = ambientLighting();',
		'                vec3 Idif = diffuseLighting(N, L);',
		'                vec3 Ispe = specularLighting(N, L, V);',
		'                samp.rgb += (Iamb + Idif + Ispe);',
		'            // }',
		'            // else { // Cook-Torrance mode',
