...
var Core = function(conf) {
    ...
    this._sos_value = [0.0, 1.0];
    this._att_value = [0.0, 1.0];
...
Core.prototype.init = function() {
    ...
    this._controls.dynamicDampingFactor = 0.3;
		...
		this._controls.minDistance = 0.7;
    ...
    if(this._mode == "2d") {
       ...
       uSosMinVal: { type: "f", value: this._sos_value[0] },
       uSosMaxVal: { type: "f", value: this._sos_value[1] },
       uAttMinVal: { type: "f", value: this._att_value[0] },
		   uAttMaxVal: { type: "f", value: this._att_value[1] },
...
Core.prototype.setMinSos = function(value) {
    console.log("Core: setSosMinValue()");
    this._sos_value[0] = value;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uSosMinVal", this._sos_value[0]);
    }
};

Core.prototype.setMaxSos = function(value) {
    console.log("Core: setMaxSos()");
    this._sos_value[1] = value;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uSosMaxVal", this._sos_value[1]);
    }
};

Core.prototype.setMinAtten = function(value) {
    console.log("Core: setMinAtten()");
    this._att_value[0] = value;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uAttMinVal", this._att_value[0]);
    }
};

Core.prototype.setMaxAtten = function(value) {
    console.log("Core: setMaxAtten()");
    this._att_value[1] = value;
    if(this._mode == "3d") {
        this._secondPassSetUniformValue("uAttMaxVal", this._att_value[1]);
    }
};
...
window.VRC.Core.prototype._shaders.secondPassNormalFusion = {
	uniforms: THREE.UniformsUtils.merge([
		...
		"uMinGrayVal" : { type: "f", value: -1 },
		"uMaxGrayVal" : { type: "f", value: -1 },
		"uSosMinVal" : { type: "f", value: -1 },
		"uSosMaxVal" : { type: "f", value: -1 },
		"uAttMinVal" : { type: "f", value: -1 },
		"uAttMaxVal" : { type: "f", value: -1 },
		"uSlicemapWidth" : { type: "f", value: -1 },
		"l" : { type: "f", value: -1 },
		}
	]),
	...
	fragmentShader: [
		...
		'uniform float uMinGrayVal;',
		'uniform float uMaxGrayVal;',
		'uniform float uSosMinVal;',
		'uniform float uSosMaxVal;',
		'uniform float uAttMinVal;',
		'uniform float uAttMaxVal;',
		'uniform float uSlicemapWidth;',
		'uniform float l;',
		'//Acts like a texture3D using Z slices and trilinear filtering.',
		'vec3 getVolumeValue(vec3 volpos) {',
		...
		'            posAlongN = vpos.xyz;',
		'            for (int j = 0; j < 72; j++) {',
		'              if (gray_val.r > uSosMinVal && gray_val.r < uSosMaxVal) {',
		'                surfColor = vec3(0.8, 0.2, 0.0);',
		'                break;',
		'              }',
		'              if (gray_val.g > uAttMinVal && gray_val.g < uAttMaxVal) {',
